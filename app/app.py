...
def main():
    ...
    description, temp, humidity, pressure, speed, away, city = get_weather(user)
...
def get_weather(user):
    ...
    time = requests.get('https://timeapi.io/api/Time/current/zone?timeZone=Asia/Manila').json().get('time')
    # do also for home
    return (description, temp, humidity, pressure, speed, time, name)
